...
func ToBedrockTitanEmbeddingRequest(bifrostReq *schemas.BifrostEmbeddingRequest) (*BedrockTitanEmbeddingRequest, error) {
	...
	if bifrostReq.Params != nil {
		...
		// first-class struct fields. Keep an invalid embeddingTypes value in
		...
		// Only exclude inputImage when it was actually lifted (string case);
		// non-string values stay in ExtraParams for passthrough.
		if len(bifrostReq.Params.ExtraParams) > 0 {
			...
			for k, v := range bifrostReq.Params.ExtraParams {
				...
				if k == "embeddingTypes" && embeddingTypesExtracted {
					continue
				}
...
func resolveNovaImage(ctx context.Context, model, image, detailLevel string) (*BedrockNovaEmbeddingImage, error) {
	src, err := convertImageToBedrockSource(ctx, model, image)
	if err != nil {
...
func buildNovaEmbeddingRequests(ctx context.Context, bifrostReq *schemas.BifrostEmbeddingRequest) ([]*BedrockNovaEmbeddingRequest, error) {
	...
	for _, image := range novaCollectImageInputs(extra) {
		img, err := resolveNovaImage(ctx, bifrostReq.Model, image, detailLevel)
		if err != nil {
